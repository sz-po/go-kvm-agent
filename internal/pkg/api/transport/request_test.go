...
func TestParseRequest(t *testing.T) {
	t.Run("parses full request with path, query, headers and body", func(t *testing.T) {
		...
		assert.Equal(t, "test-machine", result.PathParam["machineId"])
		assert.Equal(t, "test-peripheral", result.PathParam["peripheralId"])
		assert.Equal(t, "value1", result.Query["query1"])
		assert.Equal(t, "value2", result.Query["query2"])
		assert.Equal(t, "application/json", result.Header[transportSDK.HeaderContentType])
	...
	t.Run("parses request with empty path and query", func(t *testing.T) {
		...
		assert.Empty(t, result.PathParam)
	...
	t.Run("takes first value from multi-value headers", func(t *testing.T) {
		...
		assert.Equal(t, "text/html", result.Header[transportSDK.HeaderAccept])
	})
	...
	t.Run("handles chi route context with multiple path parameters", func(t *testing.T) {
		...
		assert.Equal(t, "v1", result.PathParam["version"])
		assert.Equal(t, "machine-123", result.PathParam["machineId"])
		assert.Equal(t, "peripheral-456", result.PathParam["peripheralId"])
		assert.Equal(t, "start", result.PathParam["actionId"])
	})
}
