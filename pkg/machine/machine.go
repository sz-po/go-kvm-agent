...
// Invalid examples: "My-LocalMachine", "my machine", "-start", "end-", "double--dash", ""
...
// NewMachineId creates and validates a new MachineId.
...
func NewMachineId(id string) (MachineId, error) {
	...
	if !machineNameRegex.MatchString(id) {
		return "", errors.New("machine id must be in kebab-case format (lowercase letters, digits, and hyphens only)")
	}

	return MachineId(id), nil
}

// String returns the string representation of the MachineId.
...
func (id *MachineId) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &idStr); err != nil {
		return fmt.Errorf("unmarshal machine id: %w", err)
	}

	validated, err := NewMachineId(idStr)
	if err != nil {
		return fmt.Errorf("unmarshal machine id: %w", err)
	}
...
// MarshalJSON implements json.Marshaler interface.
func (id MachineId) MarshalJSON() ([]byte, error) {
