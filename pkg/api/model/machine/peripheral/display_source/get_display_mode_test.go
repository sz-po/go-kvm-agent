...
func TestParseGetDisplayModeRequestPath(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			requestPath, err := ParseGetDisplayModeRequestPath(testCase.path)
...
func TestParseGetDisplayModeRequest(t *testing.T) {
	machineId, err := machineSDK.NewMachineId("machine-1")
	assert.NoError(t, err)
	peripheralId, err := peripheralSDK.NewPeripheralId("peripheral-1")
	assert.NoError(t, err)
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			request, err := ParseGetDisplayModeRequest(testCase.request)
